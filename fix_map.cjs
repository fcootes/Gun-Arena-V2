...
const startIdx = lines.findIndex(l => l === '  } else {');
const endIdx = lines.findIndex(l => l === '  // Ground Pickups - Updated to support Pistol (Type Index 3)');

if (startIdx !== -1 && endIdx !== -1) {
  const newMap = `    // 5-SECTOR SUBTERRANEAN FACILITY (LINEAR PROGRESSION)
...
`;
  ...
  const newLines = lines.slice(0, startIdx + 1).concat(newMap.split('\n')).concat(lines.slice(endIdx - 1));
  fs.writeFileSync('src/world.ts', newLines.join('\n'));
  console.log('Map correctly replaced!');
} else {
